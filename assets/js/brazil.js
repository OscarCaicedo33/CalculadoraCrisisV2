...
/*=============== Guardamos los constantes que van a ser comparables  ===============*/        
    ...
    if (alcance <= 17435 && engagement <= 195) {
    nivelRiesgo = "Level 1";
    } else if ((alcance >= 17436 && alcance <= 22814) && (engagement >= 196 && engagement <= 358)) {
    nivelRiesgo = "Level 2";
    } else if (alcance >= 22815 && engagement >= 359) {
    nivelRiesgo = "Level 3";
    } else if (alcance > 17436 || engagement > 196) { // <-- Nuevo else if
    const x = (temaSensible === 'yes' && mencionaMarca === 'yes') ? 1 : 0;
    const y = (alcance > 17436) ? 1 : 0; // <-- Valor nivel 2
    const z = (engagement > 196) ? 1 : 0; // <-- Valor nivel 2
    nivelRiesgo = x + y + z;
